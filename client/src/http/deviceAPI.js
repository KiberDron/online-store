...
export const fetchDevices = async (typeId, brandId, page, limit, orderCol = 'name', orderType = 'ASC') => {
  const {data} = await $host.get('api/device', {params: {
    typeId, brandId, page, limit, orderCol, orderType
  }})
  return data
}
...
export const fetchUserRating = async (userId, deviceId) => {
  const {data} = await $authHost.get('api/rating/user_rating/' + deviceId, {params: {
    userId
  }})
  return data
}
