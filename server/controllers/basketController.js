...
class BasketController {
  ...
  async getUserBasket(req, res) {
    ...
    let devicesIds = [];
    userBasket.forEach((basketDevice) =>
      devicesIds.push(basketDevice.deviceId)
    );
    const countSameDevices = devicesIds.reduce(
      (acc, val) => ({
        ...acc,
        [val]: (acc[val] || 0) + 1,
      }),
    ...
    const ord = [
      sequelize.literal(
        `ARRAY_POSITION(ARRAY[${devicesIds}]::integer[], "id")`
      ),
    ];
